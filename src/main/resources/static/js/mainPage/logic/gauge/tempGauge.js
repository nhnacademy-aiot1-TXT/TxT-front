function updateTemperature() {
    ...
    var url = 'http://133.186.134.250:8300/api/sensor/temperature'; // Ensure the endpoint is correct and accessible
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            var gaugeElement = document.getElementById('temp');
            ...
            console.error("Failed to fetch temperature data. Status:", xhr.status);
        }
    };
    xhr.onerror = function() {
        console.error("An error occurred during the request to fetch temperature data.");
    };
    xhr.send();
}
...
document.addEventListener('DOMContentLoaded', function() {
    updateTemperature();
    setInterval(updateTemperature, 5000); // Update temperature every 1 second
});
