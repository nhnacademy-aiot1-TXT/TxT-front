function updateco2() {
    ...
    var url = 'https://133.186.151.22:8000/api/sensor/co2'; // Ensure the endpoint is correct and accessible
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            var gaugeElement = document.getElementById('co2');
            ...
            console.error("Failed to fetch co2 data. Status:", xhr.status);
        }
    };
    xhr.onerror = function() {
        console.error("An error occurred during the request to fetch co2 data.");
    };
    xhr.send();
}
...
document.addEventListener('DOMContentLoaded', function() {
    updateco2();
    setInterval(updateco2, 5000); // Update co2 every 1 second
});
